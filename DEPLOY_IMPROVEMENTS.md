# 🚀 Улучшения deploy.sh

## Что нового в deploy.sh

Скрипт деплоя теперь **умнее** и делает всё автоматически!

### ✅ Автоматические проверки после деплоя

После обновления кода на сервере скрипт:

1. **Перезапускает nginx** - избегает 502 Bad Gateway ошибок
2. **Проверяет Backend** - `curl http://localhost/health`
3. **Проверяет Mini App** - `curl https://app.flurai.ru`
4. **Проверяет Admin Panel** - `curl https://admin.flurai.ru`
5. **Показывает статус** каждого сервиса (✅ или ⚠️)

### Пример вывода:

```bash
./deploy.sh "feat: новая функция"

🚀 Начинаем деплой...
📝 Изменения:
 M backend/app/api/sellers.py
💾 Коммитим и пушим...
✅ Код отправлен в GitHub
🔄 Обновляем сервер...
[... Docker build logs ...]
🔧 Перезапускаем nginx...
🔍 Проверка сервисов...
✅ Backend: OK
✅ Mini App: OK
✅ Admin Panel: OK

✅ Деплой завершён!

🌐 Проверьте сайты:
   https://app.flurai.ru
   https://admin.flurai.ru
```

---

## 🔧 Что исправлено

### Проблема: 502 Bad Gateway после деплоя

**Было:**
После деплоя иногда nginx не мог подключиться к upstream контейнерам (miniapp, admin), показывая 502 ошибку.

**Стало:**
Скрипт автоматически перезапускает nginx после обновления контейнеров - проблема решена!

```bash
# Автоматический перезапуск nginx
ssh yandex-cloud "docker compose -f ~/flurai/docker-compose.prod.yml restart nginx"
```

---

## 📊 Health Checks

### Backend Health Check

```bash
curl http://localhost/health
```

**Ожидаемый ответ:**
```json
{
  "status": "healthy",
  "version": "1.0.0",
  "checks": {
    "database": "ok",
    "redis": "ok"
  }
}
```

### Frontend Health Checks

```bash
# Mini App
curl -I https://app.flurai.ru
# Должен вернуть: HTTP/2 200

# Admin Panel
curl -I https://admin.flurai.ru
# Должен вернуть: HTTP/2 200
```

---

## 🎯 Использование

### Обычный деплой:

```bash
./deploy.sh "feat: добавил корзину"
```

### Что произойдёт:

1. Коммит изменений
2. Push в GitHub
3. SSH на сервер
4. Git pull
5. Docker build (backend, bot, admin, miniapp)
6. Docker up -d (перезапуск контейнеров)
7. **Nginx restart** ⭐ (новое!)
8. **Health checks** ⭐ (новое!)
9. Вывод статуса каждого сервиса

---

## ⚠️ Если проверка не прошла

### Backend показывает ⚠️

```bash
# Проверить логи
ssh yandex-cloud 'cd ~/flurai && docker compose -f docker-compose.prod.yml logs --tail 50 backend'

# Проверить статус
ssh yandex-cloud 'docker compose -f ~/flurai/docker-compose.prod.yml ps backend'

# Перезапустить
ssh yandex-cloud 'docker compose -f ~/flurai/docker-compose.prod.yml restart backend'
```

### Mini App или Admin показывает ⚠️

```bash
# Проверить логи nginx
ssh yandex-cloud 'cd ~/flurai && docker compose -f docker-compose.prod.yml logs --tail 50 nginx'

# Проверить статус контейнеров
ssh yandex-cloud 'docker compose -f ~/flurai/docker-compose.prod.yml ps'

# Перезапустить nginx ещё раз
ssh yandex-cloud 'docker compose -f ~/flurai/docker-compose.prod.yml restart nginx'
```

---

## 💡 Дополнительные возможности

### Посмотреть логи после деплоя:

```bash
# Backend
ssh yandex-cloud 'cd ~/flurai && docker compose -f docker-compose.prod.yml logs --tail 20 backend'

# Nginx
ssh yandex-cloud 'cd ~/flurai && docker compose -f docker-compose.prod.yml logs --tail 20 nginx'

# Всё сразу
ssh yandex-cloud 'cd ~/flurai && docker compose -f docker-compose.prod.yml logs --tail 20'
```

### Проверить статус всех контейнеров:

```bash
ssh yandex-cloud 'docker compose -f ~/flurai/docker-compose.prod.yml ps'
```

### Ручной health check:

```bash
# С локальной машины
curl https://api.flurai.ru/health

# Или на сервере
ssh yandex-cloud "curl http://localhost/health"
```

---

## 🔄 Сравнение: Было vs Стало

### Было (старая версия):

```bash
./deploy.sh "feat: новая функция"
# ... деплой ...
✅ Деплой завершён!

# Нужно было вручную:
# 1. Проверять работает ли сайт
# 2. Смотреть логи
# 3. Иногда перезапускать nginx из-за 502
```

### Стало (новая версия):

```bash
./deploy.sh "feat: новая функция"
# ... деплой ...
🔧 Перезапускаем nginx...
🔍 Проверка сервисов...
✅ Backend: OK
✅ Mini App: OK
✅ Admin Panel: OK
✅ Деплой завершён!

# Всё работает сразу!
# Не нужно ничего проверять вручную
```

---

## 📈 Преимущества

1. ✅ **Надёжность** - автоматический перезапуск nginx
2. ✅ **Проверка** - health checks после деплоя
3. ✅ **Наглядность** - видно статус каждого сервиса
4. ✅ **Скорость** - не нужно проверять вручную
5. ✅ **Безопасность** - сразу видно если что-то сломалось

---

## 🎓 Best Practices

### 1. Всегда проверяйте вывод скрипта

Если видите ⚠️ - проверьте логи:

```bash
ssh yandex-cloud 'cd ~/flurai && docker compose -f docker-compose.prod.yml logs --tail 50'
```

### 2. Перед деплоем тестируйте локально

```bash
# Локально
docker-compose up -d
# Тестируем...
# Всё работает? Деплоим!
./deploy.sh "feat: ..."
```

### 3. Следите за статусом в выводе

```
✅ Backend: OK          → Всё хорошо
✅ Mini App: OK         → Всё хорошо
⚠️  Admin Panel: ...    → Нужно проверить!
```

---

## 🆘 Troubleshooting

### Деплой прошёл, но сайт не работает

1. **Подождите 5-10 секунд** - контейнеры могут запускаться
2. **Обновите страницу** в браузере (F5 или Cmd+R)
3. **Hard refresh** - Cmd+Shift+R (очистит кэш)
4. **Проверьте логи** - `ssh yandex-cloud '... logs'`

### Health check показывает ⚠️ но сайт работает

Это может быть из-за задержки сети при проверке. Проверьте вручную:

```bash
curl https://app.flurai.ru
# Если отвечает - всё ОК
```

### Все проверки ⚠️

Скорее всего проблема с сервером или сетью:

```bash
# Проверить что сервер доступен
ssh yandex-cloud "echo 'Server is reachable'"

# Проверить статус контейнеров
ssh yandex-cloud 'docker compose -f ~/flurai/docker-compose.prod.yml ps'
```

---

## 📚 См. также

- **Основная инструкция**: `SIMPLE_SOLUTION.md`
- **Команды сервера**: `SERVER_COMMANDS.md`
- **Главный гайд**: `CLAUDE.md`

---

**Создано:** 2025-02-15
**Версия:** 2.0.0
